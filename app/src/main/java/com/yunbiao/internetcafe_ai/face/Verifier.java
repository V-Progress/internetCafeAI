...
public class Verifier extends AsyncTask<Void,Void, Verifier.CompareResult> {
    ...
    //等待活体时间
    private final long WAIT_LIVENESS_TIME = 10000;
    private long waitLivnessTime = 0;
    //等待温度的时间
    ...
    public static synchronized void startCompare(@NonNull VerifyCallback verifyCallback){
        if(verifier != null && !verifier.isCancelled()){
            XLog.d("准备结束对比流程：" + verifier.hashCode());
        ...
        verifier.execute();
        XLog.d("创建新的对比流程：" + verifier.hashCode());
    }
    ...
    protected CompareResult doInBackground(Void... voids) {
        while (!verifier.isCancelled()){
            ...
            int liveness = -1;
            if(verifyCallback != null){
                liveness = verifyCallback.livness();
            }
            if(liveness != LivenessInfo.ALIVE){
                long currentTimeMillis = System.currentTimeMillis();
                if(waitLivnessTime == 0){
                    waitLivnessTime = currentTimeMillis;
                } else if(currentTimeMillis - waitLivnessTime > WAIT_LIVENESS_TIME){
                    waitLivnessTime = 0;
                    return new CompareResult(-5);
                }
                continue;
            }
            waitLivnessTime = 0;
            ...
            for (int i = 0; i < WAIT_FEATURE_TIME; i++) {
                ...
                if(verifyCallback != null){
                    ...
                    XLog.d("对比结果：%s", (faceSimilar == null ? "NULL" : faceSimilar.getScore()));
                }
                //如果此时对比结果不为空且分数大于阈值则立即结束此流程
                if(faceSimilar != null && faceSimilar.getScore() >= SIMILAR){
                    faceSimilar = compare;
                    XLog.d("对比结束：" + faceSimilar.getScore());
                    break;
                }
            }
    ...
    public static class CompareResult{
        private int resultCode;//1验证通过，0未验证通过，-1等待人脸超时，-2等待测温超时，-3提取卡片特征超时，-4提取人脸特征失败，-5等待活体超时，-111未知错误
